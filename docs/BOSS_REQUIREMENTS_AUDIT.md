...
| **Review Trust Engine** ("Legal Trust & Experience": multi-dimension ratings, star + satisfaction distribution, recommend %, "what people mention" themes, verified/attributed/pseudonymous/anonymous — **and combinable, e.g. "Verified Experience · Anonymous Client"** — moderation, fraud scoring, helpful votes, right of reply, reporting, sample-size banding, firm/LPO reviews) | ✓ | ✓ | ✓ | ✓ | ✓ | **LIVE — demonstrated end-to-end with dummy data, flag left off in production** | `FEATURE_REVIEWS` is switched **on in this local dev database only**, with `packages/db/scripts/seed-review-demo.ts` seeding 15 fictional demo advocates and 10 fictional demo organisations (never a real Bar Council record) so the full UI is visible — see `docs/REVIEW_SYSTEM_TEST_REPORT.md` for exactly what was clicked through. Production still requires the C-09/C-10 sign-off in `REVIEW_COMPLIANCE_MATRIX.md` before this flag is enabled for real | Get counsel sign-off, then flip `FEATURE_REVIEWS` in production and run the seed script's inverse (`--clear`) or simply never run it there |
| **Review Hub, main-nav visibility, "Rate CaseADVO" platform feedback** | ✓ | ✓ | ✓ | ✓ | ✓ | **LIVE — built this pass** | `Reviews` was entirely absent from the main navigation before this pass (the actual reported bug — the engine itself was real, just undiscoverable). Now: `Reviews` in `Header.tsx`, `/reviews` hub page with a live cross-platform feed, a homepage discovery section, and a fully separate `site_feedback` dataset/`/rate-us`/`/admin/site-feedback` for platform feedback, never mixed with professional reviews | — |
